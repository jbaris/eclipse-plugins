...
//				| SWT.APPLICATION_MODAL | SWT.RESIZE);
	...
	private PackageExplorerContentProvider createContentProvider() {
		if (model instanceof WorkingSetModel) {
			return new WorkingSetAwareContentProvider(true,
	...
	protected Control createDialogArea(Composite parent) {
		...
		final int styleBits = SWT.SINGLE | SWT.H_SCROLL | SWT.V_SCROLL
				| SWT.BORDER;
		final FilteredTree filteredTree = new FilteredTree(composite,
				styleBits, filter, false) {
			...
			protected WorkbenchJob doCreateRefreshJob() {
				...
				refreshJob.addJobChangeListener(new JobChangeAdapter() {
					...
					public void done(IJobChangeEvent event) {
						final boolean hasItems = treeViewer.getTree()
								.getItems().length > 0;
		...
		onlyJavaProjectsButton.setLayoutData(new GridData(SWT.FILL, SWT.CENTER,
				true, false));
		onlyJavaProjectsButton.setText("Only Java Projects");
		onlyJavaProjectsButton.addSelectionListener(new CheckSelectionListener(
				projectsFilter, filteredTree));
	...
	private IBaseLabelProvider createLabelProvider(
			PackageExplorerContentProvider contentProvider) {
	...
	public Object getSelectedProject() {
		final Object selected = ((TreeSelection) treeViewer.getSelection())
				.getFirstElement();
	...
	private void restoreWidgetValues() {
		...
		final Boolean onlyJavaProjects = settings
				.getBoolean(ONLY_JAVA_PROJECTS_ID);
	...
	public void saveWidgetValues() {
		final IDialogSettings settings = getDialogSettings();
		settings.put(ONLY_JAVA_PROJECTS_ID, onlyJavaProjectsButton
				.getSelection());
	}
